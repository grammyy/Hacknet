...
profiles={
    load:function(account){
        ...
        CMD=new Map([
            ["help",["SYS.compile(['?:>[]']);SYS.compile([...[\""+"-".repeat(innerWidth/19)+"\"],...eval(\"switch(::){case 2:['yo'];default:['Command list - Page 1 of 3','','help [PAGE NUMBER]','  Displays the specified page of commands.','','scp [filename] [OPTIONAL: destination]','  copies file named [filename] from remote machine to specified local folder (/bin default)','','scan','  Scans for links on the connected machine and adds them to the map','','rm [filename (or use * for all files in folder)]','  Deletes specified file(s)','','ps','  Lists currently running processes and their PIDs','','kill [PID]','  Kills Process number [PID]','','ls','  Lists all files in current directory','','cd [foldername]','  Moves current working directory to the specified folder','','mv [FILES] [DESTINATION]','  Moves or renames [FILE] to [DESTINATION] (i.e: mv hi.txt ../bin/hi.txt)','','connect [ip]','  Connect to an External Computer','','help [PAGE NUMBER]',' Displays the specified page of commands']}\"),...[\""+"-".repeat(innerWidth/19)+"\"]])",["'"+"-".repeat(innerWidth/18.8)+"'","'Command list - Page 1 of 3'"]]],
